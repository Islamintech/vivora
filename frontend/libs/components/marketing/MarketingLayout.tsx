...
import {
  Public, Chat, Email, VerifiedUser, Security,
  Menu as MenuIcon, Close, ExpandMore,
} from '@mui/icons-material';
import { useMarketingT, MARKETING_LOCALES, MLocale } from '../../marketing-i18n';
import Flag from '../Flag';
import Logo from '../Logo';

// ── Shared warm Material-3 palette ──
// PRIMARY is sampled straight from the logo mark; ORANGE is the lighter end
// the brand gradients fade into.
export const PRIMARY = '#FD6511';
export const ORANGE = '#FF8A3D';
// PRIMARY only clears 3:1 on our cream surface, which is enough for large text
// but not for the small uppercase eyebrows and column headings. Those use this
// darker shade instead, which clears 4.5:1.
export const PRIMARY_DEEP = '#B03D02';
    ...
    <>
      <Button
        ...
        sx={{
          ...
          '&:hover': {
            bgcolor: dark ? 'rgba(255,255,255,0.06)' : 'rgba(253,101,17,0.06)',
            borderColor: dark ? 'rgba(255,255,255,0.28)' : 'rgba(253,101,17,0.35)',
          },
        }}
        aria-label="Language"
      >
...
export function MarketingNav({ dark = false }: { dark?: boolean }) {
  ...
  const c = dark
    ? {
        ...
        drawerBg: '#0E1219', drawerText: '#E7EBF2', drawerHoverBg: 'rgba(253,101,17,0.1)',
        divider: 'rgba(255,255,255,0.1)', shadow: '0 8px 24px rgba(0,0,0,0.4)',
        grad: 'linear-gradient(90deg, #FD6511, #FF8A3D)', glow: 'rgba(253,101,17,0.35)',
      }
    : {
        barBg: 'rgba(255,248,242,0.7)', barBorder: 'rgba(140,113,100,0.1)',
        brand: PRIMARY, link: ON_VAR, linkHover: PRIMARY,
        loginColor: PRIMARY, loginBg: SURFACE, loginBorder: '2px solid rgba(253,101,17,0.2)',
        loginHoverBorder: 'rgba(253,101,17,0.4)', loginHoverBg: 'rgba(253,101,17,0.05)',
        drawerBg: SURFACE, drawerText: ON_SURFACE, drawerHoverBg: 'rgba(253,101,17,0.06)',
        divider: 'rgba(140,113,100,0.15)', shadow: '0 8px 24px rgba(253,101,17,0.06)',
        grad: GRAD, glow: 'rgba(253,101,17,0.3)',
      };
      ...
      <Container maxWidth="xl">
        <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={2}>
          ...
          <Stack component={NextLink} href="/" direction="row" alignItems="center" spacing={1.5} sx={{ textDecoration: 'none', flexShrink: 0, '&:hover .lg': { transform: 'rotate(-6deg) scale(1.05)' } }}>
            <Box className="lg" sx={{ display: 'flex', alignItems: 'center', transition: 'transform .35s cubic-bezier(0.34,1.4,0.64,1)', '& img': { width: { xs: 42, md: 50 }, height: { xs: 42, md: 50 } } }}>
              <Logo size={50} />
            </Box>
              ...
              return (
                <Typography
                  ...
                  sx={{
                    ...
                    transition: 'color .2s',
                    // No hover plate - the sliding underline below is the only
                    // hover affordance.
        ...
        <Stack spacing={0.5}>
          {navLinks.map((l) => (
            <Typography key={l.href} component={NextLink} href={l.href} onClick={() => setOpen(false)}
              sx={{ fontSize: 16, fontWeight: 700, color: c.drawerText, textDecoration: 'none', py: 1.5, px: 1.5, borderRadius: 2, '&:hover': { bgcolor: c.drawerHoverBg, color: dark ? '#FF8A3D' : PRIMARY } }}>
              {l.label}
            </Typography>
          ))}
        </Stack>
            ...
            <Grid container spacing={4}>
              {footerCols.map((col) => (
                <Grid item xs={6} sm={4} key={col.h}>
                  <Typography sx={{ fontSize: 10, fontWeight: 700, color: PRIMARY_DEEP, mb: 2, textTransform: 'uppercase', letterSpacing: '0.15em' }}>{col.h}</Typography>
                  <Stack spacing={1.5}>
