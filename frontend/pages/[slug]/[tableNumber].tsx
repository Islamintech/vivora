...
import { PLACE_ORDER_MUTATION, SUBMIT_FEEDBACK_MUTATION } from '../../apollo/user/mutation';
...
import WelcomeGate, { OrderType } from '../../libs/components/customer/WelcomeGate';
import ItemSheet from '../../libs/components/customer/ItemSheet';
      ...
      <>
        ...
        <Box sx={{ minHeight: '100vh', bgcolor: '#FAFAFA', maxWidth: 480, mx: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', p: 3 }}>
          <Card sx={{ width: '100%', textAlign: 'center' }}>
            <CardContent sx={{ p: 4 }}>
              {restaurant.logo
                ? <Box component="img" src={sized(restaurant.logo, 160)} alt="" onError={onImageError(restaurant.logo)} sx={{ width: 64, height: 64, borderRadius: 2, objectFit: 'cover', mb: 2 }} />
    ...
    <>
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: '#FAFAFA', maxWidth: 480, mx: 'auto', position: 'relative' }}>
        ...
        <CoverHeader image={restaurant?.coverImage} sx={{ pt: 4, pb: 6, px: 2.5 }}>
          ...
          {restaurant?.logo && (
            <Avatar src={sized(restaurant.logo, 120)} sx={{ width: 56, height: 56, mb: 1.5, mx: 'auto' }} />
          )}
                            ...
                            {cartItem && (
                              <Box
                                sx={{
                                  ...
                                  boxShadow: '0 2px 8px rgba(253,101,17,0.5)',
                                }}
                              >
                                {cartItem.quantity}
                              </Box>
                            ...
                            <Stack direction="row" alignItems="flex-start" spacing={0.75} mb={0.3}>
                              ...
                              {item.isBestSeller ? (
                                <Chip
                                  label={`🔥 ${t.bestSeller}`}
                                  size="small"
                                  sx={{ height: 20, fontSize: '0.66rem', fontWeight: 800, bgcolor: '#FFF1E4', color: '#B03D02', flexShrink: 0 }}
                                />
                              ) : item.isPopular ? (
              ...
              {cartCount > 0 && (
                <Button
                  ...
                  sx={{
                    ...
                    boxShadow: '0 8px 30px rgba(253,101,17,0.4)',
