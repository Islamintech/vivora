...
export class TranslationService {
  ...
  async translateMenuText(name: string, description = ''): Promise<TranslationMap> {
    ...
    const prompt = [
      ...
      '- Write the name in the target language\'s own script. Russian names use',
      '  Cyrillic and Korean names use Hangul, always. Never leave an Uzbek',
      '  name in Latin letters for ru or ko, and do this for every word of a',
      '  multi-word name: "Qovurma Lagmon" becomes "Ковурма Лагман" and',
      '  "볶음 라그만". Latin may stay only inside a brand name such as KFC.',
      '- Russian must use only the 33 letters of the Russian alphabet. Uzbek',
      '  Cyrillic letters such as Қ, Ғ, Ў, Ҳ do not exist in Russian - write',
      '  "Ковурма", never "Қовурма".',
      '- A note in brackets is not part of the name. Translate it: "(1 ta six)"',
      '  becomes "(1 шампур)" and "(1꼬치)", "(faqat zakaz)" becomes',
      '  "(только на заказ)" and "(주문 전용)".',
